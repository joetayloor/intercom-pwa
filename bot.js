...
function httpsGet(url) {
  return new Promise((resolve, reject) => {
    httpsLib.get(url, {
      headers: { 'User-Agent': 'flightintercom1/8 CFNetwork/1496.0.7 Darwin/23.5.0' }
    }, res => {
      let data = '';
...
function downloadImage(imgurl) {
  return new Promise((resolve, reject) => {
    ...
    httpsLib.get({
      ...
      port: 443,
      headers: {
        'User-Agent': 'flightintercom1/8 CFNetwork/1496.0.7 Darwin/23.5.0',
        'Accept': '*/*'
      }
    }, res => {
      const chunks = [];
...
function sendMessage(text, extra) {
  extra = extra || {};
  return Promise.all(CHAT_IDS.map(id => tgCall('sendMessage', Object.assign({ chat_id: id, text: text, parse_mode: 'HTML' }, extra))));
}

function sendMessageTo(chatId, text, extra) {
  extra = extra || {};
  return tgCall('sendMessage', Object.assign({ chat_id: chatId, text: text, parse_mode: 'HTML' }, extra));
}

function answerCallback(callback_query_id, text) {
  return tgCall('answerCallbackQuery', { callback_query_id: callback_query_id, text: text });
}

// ── Keyboards ──
function doorsKeyboard() {
  return { inline_keyboard: Object.entries(DOORS).map(function(entry) {
    return [{ text: '🔓 ' + entry[1], callback_data: 'open_' + entry[0] }];
  })};
}
...
function smartKeyboard(eventName) {
  ...
  if (matchedId) {
    ...
    Object.entries(DOORS).forEach(function(entry) {
      buttons.push([{ text: '🔓 ' + entry[1], callback_data: 'open_' + entry[0] }]);
    });
  }
...
function openDoor(doorId) {
  ...
  return new Promise(function(resolve) {
    httpsLib.get(url, { headers: { 'User-Agent': 'flightintercom1/8 CFNetwork/1496.0.7 Darwin/23.5.0' } }, function(res) {
      let body = '';
      res.on('data', function(c) { body += c; });
      res.on('end', function() {
        console.log('🔑 sendtask статус: ' + res.statusCode + ' тело: ' + body);
      ...
      if (e.type === 'call') {
        ...
        if (!imgurl || imgurl.includes('default.jpg')) {
          try {
            ...
            const freshEvent = (fresh && fresh.eventarray ? fresh.eventarray : []).find(function(fe) { return fe.eventid === e.eventid; });
        ...
        if (imgurl && !imgurl.includes('default.jpg')) {
          try {
            ...
            await Promise.all(CHAT_IDS.map(function(id) { return sendPhotoBuffer(id, buffer, caption, keyboard); }));
          } catch(err) {
  ...
  try {
    ...
    for (const update of data.result) {
      ...
      if (update.callback_query) {
        ...
        console.log('📲 Callback: ' + cbData + ' от ' + fromId);
        ...
        if (cbData.startsWith('open_')) {
          ...
          setTimeout(function() { delete openLocks[doorId]; }, 5000);
